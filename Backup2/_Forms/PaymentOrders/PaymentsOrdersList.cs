...
//					this.daUseProc.SelectCommand.Parameters["@OrgNameOwner"].Value	=this.cmbInnerOrg.Text + "%";
		...
		{
			BindingManagerBase bm = this.BindingContext[this.dataView1];
			if(bm.Position == -1)
				return;
			BPS.BLL.PaymentOrders.DataSets.dsPaymentOrderList.PaymentsOrdersListRow rw = (BPS.BLL.PaymentOrders.DataSets.dsPaymentOrderList.PaymentsOrdersListRow) ((DataRowView) bm.Current).Row;
			RepDoc rd = new RepDoc();
			rd.LoadForDataSet("PaymentOrder.rpt");
			rd.SetDataSource(this.dsPaymentOrderList1);
			rd.RecordSelectionFormula = "{PaymentsOrdersList.PaymentOrderID}=" + rw.PaymentOrderID.ToString();
